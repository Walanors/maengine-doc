...
<Accordion title="DB.Find(table, filter?) → array">

Find records matching an optional filter.
...
Subscribe to real-time database changes. Callbacks fire when records are created, updated, or deleted.
...
<Callout type="warn">
Subscriptions receive all changes, including those from other server instances or external tools. Use them for server-wide event handling, not for tracking your own writes.
</Callout>
